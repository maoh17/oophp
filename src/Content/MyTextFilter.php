...
class MyTextFilter
{
    ...
    public function parse($text, $filter)
    {
        foreach ($filter as $key) {
            $filterFunction = $this->filters[$key];

            $text = $this->$filterFunction($text);
        }
     ...
     *
     * @param string $text   The text to filter.
     * @param array  $filter Comma-separated filters to use.
     *
     * @return string with the formatted text.
     */
    public function parseCS($text, $filter)
    {
        if (!$filter or $filter == "") {
            $res = $text;
        } else {
            $filter = explode(",", $filter);
            $res = $this->parse($text, $filter);
        }
        return $res;
    }
}
